...
@section('admin_content_header')
    ...
    <x-ad-breadcrumb :items="[
        ...
        ['label' => 'Prescriptions', 'url' => route('administration.prescriptions.index')],
                    ...
                    <div class="card-body">
                        <div class="prescription-info">
                            ...
                            @if($prescription->patient)
                                <p class="mb-2"><strong>Name:</strong> {{ $prescription->patient->name ?? 'N/A' }}</p>
                                <p class="mb-2"><strong>Email:</strong> {{ $prescription->patient->email ?? 'N/A' }}</p>
                                <p class="mb-2"><strong>Phone:</strong> {{ $prescription->patient->profile->contact_no ?? 'N/A' }}</p>
                                @if($prescription->patient->profile && $prescription->patient->profile->date_of_birth)
                                    <p class="mb-2"><strong>Age:</strong> {{ \Carbon\Carbon::parse($prescription->patient->profile->date_of_birth)->age }} years</p>
                                @endif
                                @if($prescription->patient->profile && $prescription->patient->profile->blood_group)
                    ...
                    <div class="card-body">
                        <div class="prescription-info">
                            ...
                            @if($prescription->doctor && $prescription->doctor->user)
                                <p class="mb-2"><strong>Name:</strong> Dr. {{ $prescription->doctor->user->name ?? 'N/A' }}</p>
                                ...
                                @endif
                            @else
                                <p class="mb-2 text-muted">Doctor information not available</p>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <p class="mb-2"><strong>Date:</strong>
                                    @if($prescription->appointment && $prescription->appointment->appointment_date)
                                        {{ \Carbon\Carbon::parse($prescription->appointment->appointment_date)->format('M d, Y') }}
                                    @elseif($prescription->appointment && $prescription->appointment->start_at)
                            ...
                            <div class="col-md-6">
                                @if($prescription->appointment && $prescription->appointment->start_at && $prescription->appointment->end_at)
        ...
        <div class="row">
            <div class="col-12">
                <div class="doctor-signature text-center">
                    ...
                    @if($prescription->doctor && $prescription->doctor->user)
                        <h5 class="mb-1">Dr. {{ $prescription->doctor->user->name ?? 'Unknown Doctor' }}</h5>
                        <p class="mb-2">{{ $prescription->doctor->specialty ?? 'General Medicine' }}</p>
                    @else
                        <h5 class="mb-1 text-muted">Doctor information not available</h5>
                    @endif
